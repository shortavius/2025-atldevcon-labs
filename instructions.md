...
Which should let the user know that the next part off the command is <delay>.
This will allow the changing and querying of the LED delay time. The message
displayed should look something like this:
...
Next, we are going to add the processing of the following help command

    hrtbt delay ?

This command should let the user know that the <get> and <set> commands are
the next part of the command. The help should look something like this
    get - display the heartbeat LED delay value
    set - update the heartbeat LED delay value

## Lab 05-D

Now it is time to fully implement a command. The particular command is this

    hrtbt delay get

This command will grab the delay value from the heartbeat module in
milliseconds.

## Lab 05-E

Second to last, we need to implement the following help command

    hrtbt delay set ?

This will let the user know that they need to enter an integer representing
milliseconds of delay between the changing of LED states. The help message will
be something like

    <int> - this is an integer representing the number of milliseconds to wait
            before toggling the LED state.

## Lab 05-E

Finally, we need to implement the command that actually updates the heartbeat
delay time. That command looks like this

    hrtbt delay set <int>

Where
    <int> - is an integer value that represents the number of milliseconds to
            wait prior to changing the LED state.
